...
export function TicketSheet({
  ...
  // Initialized from the ticket; the parent remounts this via `key={ticket.id}`
  // so no effect-based sync is needed.
      ...
      <SheetContent>
        ...
        <div className="flex h-14 flex-none items-center justify-between border-b border-border px-[18px]">
          <div className="flex items-center gap-2.5 font-mono text-[12px] text-muted">
            ...
            {projectName} <span className="opacity-50">/</span> {ticket.code}
          </div>
          ...
          <div className="grid grid-cols-[88px_1fr] items-center gap-x-3.5 gap-y-2.5 text-[13px]">
            ...
            <div className="flex flex-wrap gap-1.5">
              {TICKET_STATUS_LIST.map((s) => {
                const on = ticket.status === s.id;
                return (
                  <button
                    key={s.id}
                    onClick={() => update.mutate({ id: ticket.id, status: s.id })}
                    className={cn(
            ...
            <div className="flex flex-wrap gap-1.5">
              {TICKET_PRIORITY_LIST.map((p) => {
                const on = ticket.priority === p.id;
                return (
                  <button
                    key={p.id}
                    onClick={() => update.mutate({ id: ticket.id, priority: p.id })}
                    className={cn(
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              {members.map((m) => {
                const on = ticket.assigneeId === m.id;
                return (
                  <button
                    key={m.id}
                    title={m.name}
                    onClick={() =>
                      update.mutate({ id: ticket.id, assigneeId: on ? null : m.id })
                    }
